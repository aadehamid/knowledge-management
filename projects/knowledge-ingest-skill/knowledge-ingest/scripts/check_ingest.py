...
def check_okf(vault: Path, repo: Path, validator: str | None = None):
    """Run the project's bundle validator when it declares one."""
    script = Path(validator) if validator else repo / "scripts" / "test_okf_bundle.py"
    if not script.exists():
        warn(f"no bundle validator at {script} — schema conformance not checked. "
             f"Set `validator` in knowledge-ingest.config.json or pass --validator.")
        return
...
def main():
    ...
    ap.add_argument("--validator", help="path to the project's bundle validator")
    ...
    check_okf(vault, repo, args.validator)
    check_links(vault)
