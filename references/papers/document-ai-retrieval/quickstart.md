title: DeepDive Quick Start - DeepDive
description: DeepDive

# DeepDive Quick Start {#deepdive-quick-start}
...
## Launching or Installing DeepDive {#launching-or-installing-deepdive}

### Quick launching {#quick-launching}

First, you can quickly [launch DeepDive](http://deepdive.stanford.edu/installation) with minimal installation using [Docker](https://docker.io) by running the following command:.
...
Now, point your web browser to [a terminal](http://0.0.0.0:8888/terminals/1) with shell access to an environment where DeepDive is installed. You will find our examples included there as well.

```bash
cd deepdive-examples/spouse
```

You can also see a notebook version of [the spouse example tutorial](http://deepdive.stanford.edu/example-spouse) if you point your browser to the [tutorial notebook](http://0.0.0.0:8888/notebooks/deepdive-examples/spouse/DeepDive%20Tutorial%20-%20Extracting%20mentions%20of%20spouses%20from%20the%20news.ipynb).

### Quick installation {#quick-installation}

If you cannot or do not want to use Docker for any reason, you can quickly [install DeepDive](http://deepdive.stanford.edu/installation) by selecting the `deepdive` option:
...
Alternatively, if you have access to a database server, you can configure how to access it as a URL in [the application's `db.url` file](http://deepdive.stanford.edu/deepdiveapp#db-url).

## Running your first DeepDive app {#running-your-first-deepdive-app}

Now, let's see what DeepDive can do for us. We grab a copy of [the spouse example app explained in the tutorial](http://deepdive.stanford.edu/example-spouse). This app extracts mentions of spouses from [a corpus of news articles](http://research.signalmedia.co/newsir16/signal-dataset.html "The Signal Media One-Million News Articles Dataset").
...
This will download a copy of [the example app's code and data from GitHub](https://github.com/HazyResearch/deepdive/blob/master/examples/spouse/) to a folder whose name begins with `spouse_example-`. So, let's move into it:
...
You can find some of our sampled datasets under `input/`. You can also [download the full corpus](http://research.signalmedia.co/newsir16/signal-dataset.html "The Signal Media One-Million News Articles Dataset"), but let's proceed with the one that has 1000 sampled articles. Run the following command to load the sampled articles into DeepDive: `bash
...
### 2. Process input {#2-process-input}

This app [adds some useful NLP markups](http://deepdive.stanford.edu/example-spouse#1-2-adding-nlp-markups) to the English text using [Stanford CoreNLP](http://stanfordnlp.github.io/CoreNLP/). Based on the marked up *named entity recognition* (NER) tags, it can tell which parts of the text mention people's names. All pairs of names appearing in the same sentence are [considered as *candidates* for correct mentions of married couples' names](http://deepdive.stanford.edu/example-spouse#1-3-extracting-candidate-relation-mentions).
...
For supervised machine learning, the app continues with [extracting *features*](http://deepdive.stanford.edu/example-spouse#1-4-extracting-features-for-each-candidate) from the context of those candidates and [creating a training set](http://deepdive.stanford.edu/example-spouse#3-learning-and-inference-model-specification) programmatically by finding promising positive and negative examples using [*distant supervision*](http://deepdive.stanford.edu/distant_supervision).

### 3. Run the model {#3-run-the-model}

Using the processed data, the app constructs a [statistical inference model](http://deepdive.stanford.edu/inference) to predict whether a mention is a correct mention of spouses or not, estimates the parameters (i.e., learns the weights) of the model, and computes their *marginal probabilities*.
...
DeepDive provides [a suite of tools and guidelines](http://deepdive.stanford.edu/development-cycle#3-evaluate-debug) to work with the data produced by the application. For instance, below is a screenshot of an automatic interactive search interface DeepDive provides for [browsing the processed data with predicted results](http://deepdive.stanford.edu/browsing).

![Screenshot of the search interface provided by Mindbender](http://deepdive.stanford.edu/images/browsing_results.png)

## Next steps {#next-steps}

- For more details about the spouse example we just ran here, continue reading [the tutorial](http://deepdive.stanford.edu/example-spouse).
- Other parts of the documentation will help you pick up more [background knowledge](http://deepdive.stanford.edu/index#background-reading) and learn more about [how DeepDive applications are developed](http://deepdive.stanford.edu/development-cycle).
