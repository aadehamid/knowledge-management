...
* [Why GPUs?](../1-gpu-history/)
* [The GPU hardware and software ecosystem](../2-gpu-ecosystem/)
* [What problems fit to GPU?](../3-gpu-problems/)
* GPU programming concepts
  + [Different types of parallelism](#different-types-of-parallelism)
    - [Distributed- vs. Shared-Memory Architecture](#distributed-vs-shared-memory-architecture)
    - [Processes and Threads](#processes-and-threads)
  + [Exposing parallelism](#exposing-parallelism)
  + [GPU Execution Model](#gpu-execution-model)
  + [CUDA Threads, Warps, Blocks](#cuda-threads-warps-blocks)
  + [Terminology](#terminology)
  + [Exercises](#exercises)
* [Introduction to GPU programming models](../5-intro-to-gpu-prog-models/)
* [Directive-based models](../6-directive-based-models/)
* [Non-portable kernel-based models](../7-non-portable-kernel-models/)
* [Portable kernel-based models](../8-portable-kernel-models/)
* [High-level language support](../9-language-support/)
* [Multiple GPU programming with MPI](../10-multiple_gpu/)
* [Preparing code for GPU porting](../11-gpu-porting/)
* [Recommendations](../12-recommendations/)
* [Example: putting it all together](../13-examples/)
...
* GPU programming concepts
* [Edit on GitHub](https://github.com/ENCCS/gpu-programming/blob/main/content/4-gpu-concepts.rst)

---

# GPU programming concepts[](#gpu-programming-concepts "Link to this heading")
...
## Different types of parallelism[](#different-types-of-parallelism "Link to this heading")

### Distributed- vs. Shared-Memory Architecture[](#distributed-vs-shared-memory-architecture "Link to this heading")
...
![../_images/distributed_vs_shared.png](../_images/distributed_vs_shared.png)

Distributed- vs shared-memory parallel computing.[](#id4 "Link to this image")
...
### Processes and Threads[](#processes-and-threads "Link to this heading")
...
![../_images/processes-threads.png](../_images/processes-threads.png)
...
## Exposing parallelism[](#exposing-parallelism "Link to this heading")
...
[![../_images/ENCCS-OpenACC-CUDA_TaskParallelism_Explanation.png](../_images/ENCCS-OpenACC-CUDA_TaskParallelism_Explanation.png)](../_images/ENCCS-OpenACC-CUDA_TaskParallelism_Explanation.png)
...
The task parallelism implies that there are more than one independent task that, in principle, can be executed in parallel.[](#id5 "Link to this image")
...
## GPU Execution Model[](#gpu-execution-model "Link to this heading")
...
[![../_images/cpu-gpu-highway.png](../_images/cpu-gpu-highway.png)](../_images/cpu-gpu-highway.png)

Cars and roads analogy for the CPU and GPU behavior. The compact road is analogous to the CPU
(low latency, low throughput) and the broader road is analogous to the GPU (high latency, high throughput).[](#id6 "Link to this image")
...
## CUDA Threads, Warps, Blocks[](#cuda-threads-warps-blocks "Link to this heading")
...
The programmers control how many instances of ker\_axpy\_ are created and they have to make sure that all the elements are processed and also that no out of bounds accessed are happening.
...
[![../_images/THREAD_CORE.png](../_images/THREAD_CORE.png)](../_images/THREAD_CORE.png)
...
[![../_images/WARP_SMTU.png](../_images/WARP_SMTU.png)](../_images/WARP_SMTU.png)
...
[![../_images/BLOCK_SMP.png](../_images/BLOCK_SMP.png)](../_images/BLOCK_SMP.png)
...
[![../_images/Indexing.png](../_images/Indexing.png)](../_images/Indexing.png)
...
Software mapping naming[](#id7 "Link to this table")
...
| threadIdx.{x,y,z} | | get\_local\_id({0,1,2}) | nd\_item::get\_local({2,1,0}) [[1]](#syclindex) |
| blockIdx.{x,y,z} | | get\_group\_id({0,1,2}) | nd\_item::get\_group({2,1,0}) [[1]](#syclindex) |
| blockDim.{x,y,z} | | get\_local\_size({0,1,2}) | nd\_item::get\_local\_range({2,1,0}) [[1]](#syclindex) |

[1]
([1](#id1),[2](#id2),[3](#id3))

In SYCL, the thread indexing is inverted. In a 3D grid, physically adjacent threads have consecutive X (0) index in CUDA, HIP, and OpenCL, but consecutive Z (2) index in SYCL.
...
1. Independent execution units with their own memory address spaces
2. Light execution units with shared memory address spaces
3. Communication devices between separate memory units
4. Programming models for distributed memory machines
...
1. Distributing data across computational units that run in parallel, applying the same or similar operations to different data elements.
2. Distributing tasks across computational units that run in parallel, applying different operations to the same data elements.
3. Distributing data across computational units that run sequentially, applying the same operation to all data elements.
4. Distributing tasks across computational units that run sequentially, applying different operations to different data elements.
...
1. Task Parallelism
2. Data Parallelism
3. Both data and task parallelism
4. Neither data nor task parallelism
...
1. A specific section of the CPU used for memory operations.
2. A specific section of the GPU used for memory operations.
3. A type of thread that operates on the GPU.
4. A function that is executed simultaneously by tens of thousands of threads on GPU cores.
...
1. It’s when CUDA threads in the same warp access elements of the data which are adjacent in the memory.
2. It’s when CUDA threads in different warps access elements of the data which are far in the memory.
3. It’s when all threads have access to the global GPU memory.
4. It’s when threads in a warp perform different operations.
...
1. It’s used to store global memory.
2. It’s used to store all the threads in a block.
3. It can be used to “cache” data that is used by more than one thread, avoiding multiple reads from the global memory.
4. It’s used to store all the CUDA cores.
...
1. It reduces the overall performance of the GPU.
2. It ensures that there are more blocks than SMPs present on the device, helping to hide latencies and ensure high occupancy of the GPU.
3. It leads to a memory overflow in the GPU.
4. It ensures that there are more SMPs than blocks present on the device.
...
[Previous](../3-gpu-problems/ "What problems fit to GPU?")
[Next](../5-intro-to-gpu-prog-models/ "Introduction to GPU programming models")
