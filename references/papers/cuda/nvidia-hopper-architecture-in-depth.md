...
![](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/GTC2022_SXM5_01_v001_DL-1024x576.png)

Mar 22, 2022

By [Michael Andersch](https://developer.nvidia.com/blog/author/mandersch/ "Posts by Michael Andersch"), [Greg Palmer](https://developer.nvidia.com/blog/author/gpalmer/ "Posts by Greg Palmer"), [Ronny Krashinsky](https://developer.nvidia.com/blog/author/rkrashinsky/ "Posts by Ronny Krashinsky"), [Nick Stam](https://developer.nvidia.com/blog/author/nstam/ "Posts by Nick Stam"), [Vishal Mehta](https://developer.nvidia.com/blog/author/vishalm/ "Posts by Vishal Mehta"), [Gonzalo Brito](https://developer.nvidia.com/blog/author/gonzalob/ "Posts by Gonzalo Brito") and [Sridhar Ramaswamy](https://developer.nvidia.com/blog/author/sramaswamy/ "Posts by Sridhar Ramaswamy")

Like

[Discuss (3)](#entry-content-comments)

* [L](https://www.linkedin.com/sharing/share-offsite/?url=https%3A%2F%2Fdeveloper.nvidia.com%2Fblog%2Fnvidia-hopper-architecture-in-depth%2F)
* [T](https://twitter.com/intent/tweet?text=NVIDIA+Hopper+Architecture+In-Depth+%7C+NVIDIA+Technical+Blog+https%3A%2F%2Fdeveloper.nvidia.com%2Fblog%2Fnvidia-hopper-architecture-in-depth%2F)
* [F](https://www.facebook.com/sharer/sharer.php?u=https%3A%2F%2Fdeveloper.nvidia.com%2Fblog%2Fnvidia-hopper-architecture-in-depth%2F)
* [R](https://www.reddit.com/submit?url=https%3A%2F%2Fdeveloper.nvidia.com%2Fblog%2Fnvidia-hopper-architecture-in-depth%2F&title=NVIDIA+Hopper+Architecture+In-Depth+%7C+NVIDIA+Technical+Blog)
* E

## AI-Generated Summary

* The NVIDIA H100 Tensor Core GPU, based on the new NVIDIA Hopper architecture, delivers an order-of-magnitude performance leap for large-scale AI and HPC over the prior-generation NVIDIA A100 Tensor Core GPU.
* Fourth-generation Tensor Cores introduce FP8 precision and a transformer engine that together provide up to 9x faster AI training and up to 30x faster AI inference on large language models compared to A100.
* New DPX instructions accelerate dynamic programming algorithms such as Smith-Waterman for genomics and Floyd-Warshall for robotics routing by up to 7x over A100.
* The H100 SXM5 GPU is the world's first GPU with HBM3 memory, delivering over 3 TB/sec of memory bandwidth, while a 50 MB L2 cache reduces trips to DRAM.
* Second-generation Multi-Instance GPU technology provides approximately 3x more compute capacity and nearly 2x more memory bandwidth per instance, with Confidential Computing support at the MIG level.
* Fourth-generation NVLink and the new NVLink Switch System enable up to 256 GPUs to connect with 57.6 TB/sec of all-to-all bandwidth and one exaFLOP of FP8 sparse AI compute.

### Next Steps

* Read the [NVIDIA H100 Tensor Core GPU Architecture whitepaper](https://resources.nvidia.com/en-us-tensor-core/gtc22-whitepaper-hopper) for more information about new and improved H100 features.
* Explore the [CUDA cooperative groups API](https://docs.nvidia.com/cuda/cuda-programming-guide/04-special-topics/cooperative-groups.html) to leverage thread block cluster capabilities.
* Review the [NVIDIA SHARP documentation](https://docs.nvidia.com/networking/display/SHARPv200) for in-network reduction acceleration on third-generation NVSwitch.
...
![NVIDIA H100 GPU SXM5 Module](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/SXM5-White-4-NEW-FINAL-625x291.jpg)![NVIDIA H100 GPU SXM5 Module](data:image/svg+xml...)
...
![NVIDIA H100 GPU HPC and AI Preliminary Performance Chart using from 8 to 256 H100 GPUs](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Perf-Main-FINAL-625x264.jpg)![NVIDIA H100 GPU HPC and AI Preliminary Performance Chart using from 8 to 256 H100 GPUs](data:image/svg+xml...)
...
![NVIDIA GH100 GPU Full-chip block diagram](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Full-H100-GPU-with-144-SMs-625x279.png)![NVIDIA GH100 GPU Full-chip block diagram](data:image/svg+xml...)
...
![NVIDIA GH100 GPU SM (Streaming Multiprocessor) block diagram](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/H100-Streaming-Multiprocessor-SM-625x869.png)![NVIDIA GH100 GPU SM (Streaming Multiprocessor) block diagram](data:image/svg+xml...)
...
![NVIDIA Hopper H100 GPU FP16 Tensor Core structure and throughput diagram compared to NVIDIA Ampere A100 GPU FP16 Tensor Core structure and throughput](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/A100-FP16-vs-H100-FP16-625x346.png)![NVIDIA Hopper H100 GPU FP16 Tensor Core structure and throughput diagram compared to NVIDIA Ampere A100 GPU FP16 Tensor Core structure and throughput](data:image/svg+xml...)
...
![New Floating Point FP8 Tensor Core Precision formats and accumulator types](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/New-Hopper-FP8-Precisions-625x340.jpg)![New Floating Point FP8 Tensor Core Precision formats and accumulator types](data:image/svg+xml...)

*Figure 6. New NVIDIA Hopper FP8 precisions: 2x throughput and half the footprint of H100 FP16 or BF16*

![NVIDIA Hopper H100 GPU FP8 Tensor Core structure and throughput diagram compared to NVIDIA Ampere A100 GPU FP16 Tensor Core structure and throughput](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/A100-FP16-vs-H100-FP8-625x374.png)![NVIDIA Hopper H100 GPU FP8 Tensor Core structure and throughput diagram compared to NVIDIA Ampere A100 GPU FP16 Tensor Core structure and throughput](data:image/svg+xml...)

*Figure 7. H100 FP8 Tensor Core has 6x throughput compared to A100 FP16 Tensor Core*

![NVIDIA H100 GPU TF32, FP64, and INT8 Tensor Core structure and throughput diagram compared to NVIDIA A100 GPU TF22, FP64, INT8 Tensor Core structure and throughput](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/TF32-FP64-INT8-Combo-625x351.png)![NVIDIA H100 GPU TF32, FP64, and INT8 Tensor Core structure and throughput diagram compared to NVIDIA A100 GPU TF22, FP64, INT8 Tensor Core structure and throughput](data:image/svg+xml...)
...
![NVIDIA H100 GPU's new DPX Instruction usage examples and performance](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/DPX-Instructions-Accelerate-Dynamic-Programming-and-Support-Many-Algorithms-625x239.jpg)![NVIDIA H100 GPU's new DPX Instruction usage examples and performance](data:image/svg+xml...)
...
![NVIDIA H100 GPU compute performance improvement over NVIDIA A100 GPU](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/H100-Compute-Improvement-Summary-625x300.jpg)![NVIDIA H100 GPU compute performance improvement over NVIDIA A100 GPU](data:image/svg+xml...)
...
![NVIDIA H100 GPU Thread Block Clusters and Grids that include Thread Block Clusters compared to Grids of Thread Blocks](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Thread-Block-Clusters-and-Grids-with-Clusters-625x185.jpg)![NVIDIA H100 GPU Thread Block Clusters and Grids that include Thread Block Clusters compared to Grids of Thread Blocks](data:image/svg+xml...)
...
![NVIDIA A100 GPU vs H100 GPU Thread Block data exchange differences. H100 includes a direct SM-to-SM network in a Cluster.](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Thread-Block-to-Thread-Block-data-exchange-A100-vs-H100-with-Clusters-625x135.jpg)![NVIDIA A100 GPU vs H100 GPU Thread Block data exchange differences. H100 includes a direct SM-to-SM network in a Cluster.](data:image/svg+xml...)
...
![NVIDIA H100 GPU Cluster Performance Examples with three algorithms](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Cluster-versus-non-Cluster-Performance-Comparisons-625x482.jpg)![NVIDIA H100 GPU Cluster Performance Examples with three algorithms](data:image/svg+xml...)
...
![NVIDIA H100 GPU asynchronous execution enhancements overview](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Asynchronous-Execution-Concurrency-and-Enhancements-in-Hopper-625x257.jpg)![NVIDIA H100 GPU asynchronous execution enhancements overview](data:image/svg+xml...)
...
![NVIDIA H100 GPU new Tensor Memory Accelerator address generation example](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/TMA-Address-Generation-via-Copy-Descriptor-625x238.jpg)![NVIDIA H100 GPU new Tensor Memory Accelerator address generation example](data:image/svg+xml...)
...
![Comparison of NVIDIA H100 GPU new Tensor Memory Accelerator versus NVIDIA A100 GPU LDGSTS Instruction to perform memory copies](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Asynchronous-Memory-Copy-with-TMA-on-H100-vs-LDGSTS-Instruction-on-A100-625x277.jpg)![Comparison of NVIDIA H100 GPU new Tensor Memory Accelerator versus NVIDIA A100 GPU LDGSTS Instruction to perform memory copies](data:image/svg+xml...)
...
![New NVIDIA H100 GPU asynchronous transaction barrier feature compared to NVIDIA A100 GPU asynchronous barrier](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Asynchronous-Barrier-in-Ampere-vs-Asynchronous-Transaction-Barrier-in-Hopper-625x214.jpg)![New NVIDIA H100 GPU asynchronous transaction barrier feature compared to NVIDIA A100 GPU asynchronous barrier](data:image/svg+xml...)
...
![New NVIDIA H100 GPU HBM3 DRAM Bandwidth comparison to NVIDIA A100, V100, and P100 GPU HBM2 DRAM bandwidths](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Worlds-First-HBM3-GPU-Memory-Architecture-2x-Delivered-Bandwidth-625x458.png)![New NVIDIA H100 GPU HBM3 DRAM Bandwidth comparison to NVIDIA A100, V100, and P100 GPU HBM2 DRAM bandwidths](data:image/svg+xml...)
...
| GPU Boost Clock  (Not finalized for H100)3 | 1410 MHz | Not finalized | Not finalized |
...
| Memory Data Rate  (Not finalized for H100)1 | 1215 MHz DDR | Not finalized | Not finalized |
...
**Table 3. Comparison* of NVIDIA A100 and H1001 Data Center GPUs*
...
![Transformer Model Size and Complexity Increases in recent years with examples](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Transformers-Model-Sizes-Increasing-Exponentially-with-Many-Different-Use-Cases-625x260.jpg)![Transformer Model Size and Complexity Increases in recent years with examples](data:image/svg+xml...)
...
![NVIDIA H100 GPU Transformer Engine conceptual operation diagram](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Transformer-Engine-Conceptual-Operation-625x523.png)![NVIDIA H100 GPU Transformer Engine conceptual operation diagram](data:image/svg+xml...)
...
![NVIDIA's new DGX H100 SuperPOD 256 GPU clustered system performance compared to DGX A100 SuperPOD](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/DGX-A100-vs-DGX-H100-256-node-NVIDIA-SuperPOD-Architecture-Comparison-625x330.jpg)![NVIDIA's new DGX H100 SuperPOD 256 GPU clustered system performance compared to DGX A100 SuperPOD](data:image/svg+xml...)
...
[Data Center / Cloud](https://developer.nvidia.com/blog/category/data-center-cloud/) | [Networking / Communications](https://developer.nvidia.com/blog/category/networking-communications/) | [HPC / Scientific Computing](https://developer.nvidia.com/blog/recent-posts/?industry=HPC+%2F+Scientific+Computing) | [Grace CPU](https://developer.nvidia.com/blog/recent-posts/?products=Grace+CPU) | [H100](https://developer.nvidia.com/blog/recent-posts/?products=H100) | [Intermediate Technical](https://developer.nvidia.com/blog/recent-posts/?learning_levels=Intermediate+Technical) | [Tutorial](https://developer.nvidia.com/blog/recent-posts/?content_types=Tutorial) | [Confidential Compute](https://developer.nvidia.com/blog/tag/confidential-compute/) | [featured](https://developer.nvidia.com/blog/tag/featured/) | [GTC](https://developer.nvidia.com/blog/tag/gtc/) | [GTC Digital 2022](https://developer.nvidia.com/blog/tag/gtc-digital-2022/) | [Hopper](https://developer.nvidia.com/blog/tag/hopper/) | [Tensor Cores](https://developer.nvidia.com/blog/tag/tensor-cores/)

## About the Authors

![Avatar photo](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Michael-Andersch-131x131.jpg)![Avatar photo](data:image/svg+xml...)
...
[View all posts by Michael Andersch](https://developer.nvidia.com/blog/author/mandersch/)

![Avatar photo](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Greg-Palmer-131x131.jpg)![Avatar photo](data:image/svg+xml...)
...
[View all posts by Greg Palmer](https://developer.nvidia.com/blog/author/gpalmer/)
...
![Avatar photo](https://developer-blogs.nvidia.com/wp-content/uploads/2021/02/vishal-131x131.png)![Avatar photo](data:image/svg+xml...)
...
[View all posts by Vishal Mehta](https://developer.nvidia.com/blog/author/vishalm/)

![Avatar photo](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/Gonzalo-Brito-1.jpg)![Avatar photo](data:image/svg+xml...)
...
[View all posts by Gonzalo Brito](https://developer.nvidia.com/blog/author/gonzalob/)
...
![](https://developer-blogs.nvidia.com/wp-content/uploads/2024/12/nvidia-h200-nvl-1-660x370.jpg)![](data:image/svg+xml...)

### Deploying NVIDIA H200 NVL at Scale with New Enterprise Reference Architecture

[Deploying NVIDIA H200 NVL at Scale with New Enterprise Reference Architecture](https://developer.nvidia.com/blog/deploying-nvidia-h200-nvl-at-scale-with-new-enterprise-reference-architecture/)

![](https://developer-blogs.nvidia.com/wp-content/uploads/2023/05/nvidia-dgx-gh200-1-960x540.jpg)![](data:image/svg+xml...)

### Announcing NVIDIA DGX GH200: The First 100 Terabyte GPU Memory System

[Announcing NVIDIA DGX GH200: The First 100 Terabyte GPU Memory System](https://developer.nvidia.com/blog/announcing-nvidia-dgx-gh200-first-100-terabyte-gpu-memory-system/)

![The NVIDIA HGX H100 is a key GPU server building block powered by the Hopper architecture.](https://developer-blogs.nvidia.com/wp-content/uploads/2022/03/HGX-H100-8-GPU_16x9.png)![The NVIDIA HGX H100 is a key GPU server building block powered by the Hopper architecture.](data:image/svg+xml...)

### Introducing NVIDIA HGX H100: An Accelerated Server Platform for AI and High-Performance Computing

[Introducing NVIDIA HGX H100: An Accelerated Server Platform for AI and High-Performance Computing](https://developer.nvidia.com/blog/introducing-nvidia-hgx-h100-an-accelerated-server-platform-for-ai-and-high-performance-computing/)

![](https://developer-blogs.nvidia.com/wp-content/uploads/2020/05/HGX_A100_8_way.jpg)![](data:image/svg+xml...)

### Introducing NVIDIA HGX A100: The Most Powerful Accelerated Server Platform for AI and High Performance Computing

[Introducing NVIDIA HGX A100: The Most Powerful Accelerated Server Platform for AI and High Performance Computing](https://developer.nvidia.com/blog/introducing-hgx-a100-most-powerful-accelerated-server-platform-for-ai-hpc/)

![](https://developer-blogs.nvidia.com/wp-content/uploads/2018/05/HGX-2_KV_16x9_HD.jpg)![](data:image/svg+xml...)

### HGX-2 Fuses HPC and AI Computing Architectures

[HGX-2 Fuses HPC and AI Computing Architectures](https://developer.nvidia.com/blog/hgx-2-fuses-ai-computing/)
