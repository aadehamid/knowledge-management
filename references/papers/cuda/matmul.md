...
![Figure 1: Model of the NVIDIA Hopper H100 GPU](/blog/matmul/h100_model.png)
...
![Figure 2: Memory hierarchy of the H100 (SXM5) GPU](/blog/matmul/mem_hierarchy.png)
...
![Figure 3: H100 SXM5 BF16 speed-of-light derivation](/blog/matmul/h100_sol.png)
...
![Figure 4: Power throttling reduces clock frequency and lowers the effective “speed of light”](/blog/matmul/clk.png)
...
![Figure 5: CUDA Programming Model: threads, warps, blocks, clusters, grids](/blog/matmul/cuda_model.png)
...
![Figure 6: CUDA's built-in variables: how threads know where they are](/blog/matmul/cuda_model2.png)
...
![Figure 7: Inside a DRAM cell: transistor + capacitor, wordline + bitline](/blog/matmul/gmem_dram_cell.png)
...
![Figure 8: GMEM model](/blog/matmul/gmem.png)
...
![Figure 9: Effect of access pattern in GMEM](/blog/matmul/gmem_example.png)
...
![Figure 10: SMEM model](/blog/matmul/smem_pt1.png)
...
![Figure 24: New logical organization of row2 and col2 variables](/blog/matmul/simple_kernel_uncoalesced_logical.png)

Figure 24: New logical organization of row2 and col2 variables

And here's what the modified kernel does now:

![Figure 25: Naive kernel with uncoalesced GMEM access](/blog/matmul/simple_kernel_uncoalesced.png)
...
![Figure 26: Roofline model](/blog/matmul/roofline.png)
...
![Figure 27: Matmul as a sum of partial outer products](/blog/matmul/outer.png)
...
![Figure 28: Dot product is equivalent to a sum of partial dot products](/blog/matmul/block_tile_equivalence.png)
...
![Figure 29: Arithmetic intensity improves when each thread computes multiple outputs and when tiles approach a square shape](/blog/matmul/arithmetic_intensity.png)
...
![Figure 30: High-level structure of the warp-tiling algorithm, also referred to as block tiling.](/blog/matmul/warp_tiling_pt1.png)
...
![Figure 31: Loading a chunk of B (GMEM) into Bs (SMEM)](/blog/matmul/warp_tiling_pt2_smem_load_b.png)
...
![Figure 32: Loading a chunk of A (GMEM) into As (SMEM)](/blog/matmul/warp_tiling_pt2_smem_load.png)
...
![Figure 33: Performing matmul between As and Bs as a series of thread-level outer products (warp-tiling + thread-tiling).](/blog/matmul/warp_tiling_pt3_compute.png)
...
![Figure 34: We keep the same high-level structure of the warp-tiling algorithm (block-tiling).](/blog/matmul/high_level_block_structure.png)
...
3. All other threads call `bar.arrive()`, contributing their arrivals (no bytes).
...
![Figure 35: Swizzling example](/blog/matmul/swizzle_example.png)
...
![Figure 36: No bank conflicts when loading rows or columns](/blog/matmul/swizzle_property.png)
...
![Figure 37: Swizzle function intuition](/blog/matmul/swizzle_function_intuition.png)
...
![Figure 38: Connecting the swizzle function to the matrix swizzle example](/blog/matmul/swizzle_apply.png)
...
![Figure 39: Why doing four 64x16 @ 16x64 wgmma calls is equivalent to doing a 64x64 @ 64x64 matmul](/blog/matmul/4_wgmma_calls.png)
...
![Figure 40: We are wasting TMA and TC cycles - we can do better](/blog/matmul/better_pipeline.png)
...
![Figure 41: More efficient TC/TMA pipeline: producer warp-group streams tiles into a circular buffer; consumer warp-group drains tiles into tensor cores.](/blog/matmul/kernel4_pipeline.png)
...
![Figure 42: Two consumer warp groups let us grow the tile from 128x128 -> 128x256 without register spills](/blog/matmul/2wgs.png)
...
![Figure 43: Persistent kernels: overlap the output store with incoming loads by launching one long-lived block per SM that processes many tiles.](/blog/matmul/overlap_stores.png)
...
![Figure 44: Naïve schedule](/blog/matmul/naive_schedule.png)

Figure 44: Naïve schedule

Can we do better? Yes—by making the schedule cache-aware:

![Figure 45: Block-wise cache-aware schedule](/blog/matmul/blockwise_schedule.png)

Figure 45: Block-wise cache-aware schedule

But can we do even better? Surprisingly, yes—by using a space-filling curve:

![Figure 46: Hilbert-curve schedule](/blog/matmul/hilbert_schedule.png)
...
![Figure 47: Using thread block clusters to reduce the number of L2/GMEM loads.](/blog/matmul/clusters.png)
