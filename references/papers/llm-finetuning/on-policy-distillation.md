...
description: On-policy, dense supervision is a useful tool for distillation
...
![](https://thinkingmachines.ai/blog/on-policy-distillation/images/chess.png "Screenshot from chess.com. Each move is color-graded by an analysis engine, which labels moves as blunders (red), mistakes (orange), inaccuracies (yellow), or brilliant (blue).")

## On-policy distillation — best of both worlds {#on-policy-distillation--best-of-both-worlds}
...
$$
\text{KL}(\pi_{\theta}\mid\mid\pi_{\text{teacher}})=\mathbb{E}_{x\sim\pi_{\theta}}[\log⁡\pi_{\theta}(x_{t+1}∣x_{1..t})-\log⁡\pi_{\text{teacher}}(x_{t+1}∣x_{1..t})]
$$
...
In the experiments below, we generally apply on-policy distillation to models that have already been mid-trained on specific domain knowledge. This training increases the probability that the student will generate tokens within the teacher’s distribution, though it’s usually far from sufficient for replicating the teacher’s performance.SFT, using forwards KL, adds [support](https://en.wikipedia.org/wiki/Support_%28mathematics%29) for new tokens. Reverse-KL methods can then perform mode seeking within the initialization’s support. Often, as we’ll see in our personalization example, the probability of generating the relevant tokens starts at zero as the student lacks any relevant domain knowledge.
...
## Distillation for reasoning {#distillation-for-reasoning}

We use distillation to train mathematical reasoning in the [Qwen3-8B-Base](https://huggingface.co/Qwen/Qwen3-8B-Base) model, using [Qwen3-32B](https://huggingface.co/Qwen/Qwen3-32B) as a teacher model. Both the teacher (Qwen3-32B) and student (Qwen3-8B-Base) are [supported models](https://tinker-docs.thinkingmachines.ai/model-lineup) on Tinker today, so you can reproduce our experiments with the Tinker cookbook.**Update (June 2026):** The teacher (Qwen3-32B) and student (Qwen3-8B-Base) used here have since been [retired from the Tinker model lineup](https://tinker-docs.thinkingmachines.ai/tinker/model-deprecations/). The [distillation recipes in the Tinker cookbook](https://github.com/thinking-machines-lab/tinker-cookbook/tree/main/tinker_cookbook/recipes/distillation) have been updated to use [Qwen3.5-9B-Base](https://huggingface.co/Qwen/Qwen3.5-9B-Base) as the student and [Qwen3.5-9B](https://huggingface.co/Qwen/Qwen3.5-9B) as the teacher, so the experiments remain reproducible.

### Off-policy distillation {#off-policy-distillation}
...
## Distillation for personalization {#distillation-for-personalization}
...
### Training an internal assistant {#training-an-internal-assistant}
...
### Training on new knowledge degrades learned behavior {#training-on-new-knowledge-degrades-learned-behavior}
...
### On-policy distillation recovers post-training behavior {#on-policy-distillation-recovers-post-training-behavior}
...
### Dense supervision greatly improves compute efficiency {#dense-supervision-greatly-improves-compute-efficiency}
...
### Distillation can effectively reuse training data for data efficiency {#distillation-can-effectively-reuse-training-data-for-data-efficiency}
...
We repeat the above experiment of training Qwen3-8B-Base on math, but now with only a single randomly chosen prompt from the dataset.Prompt: “Evaluate the limit: $\lim⁡_{x\rightarrow∞}\sqrt{x}(\sqrt[3]{x+1}-\sqrt[3]{x-1})$”
...
### RL searches in the space of semantic strategies {#rl-searches-in-the-space-of-semantic-strategies}
...
### On-policy learning as a tool for continual learning {#on-policy-learning-as-a-tool-for-continual-learning}
