...
description: Llama from scratch I want to provide some tips from my experience implementing a paper. I'm going to cover my tips so far from implementing a dramatically...

# Llama from scratch (or how to implement a paper without crying)

   *09 Aug, 2023*   

# Llama from scratch {#llama-from-scratch}
...
## Always work iteratively: start small, stay certain, and build up. {#always-work-iteratively-start-small-stay-certain-and-build-up}
...
## Make sure your layers do what you think. {#make-sure-your-layers-do-what-you-think}
...
## Setting up our dataset {#setting-up-our-dataset}
...
\[(', or banishment,', ' or banishment, '), ('do what hands do', 'o what hands do;'), ("? If thou'lt see", " If thou'lt see "), ('and could put br', 'nd could put bre'), ("hath deliver'd.\
", "ath deliver'd.\
I"), ('ing by: whereof ', 'ng by: whereof I'), (' blows! Despisin', 'blows! Despising'), ('ng of blood, who', 'g of blood, whos')\]
...
## Setting up a working base model {#setting-up-a-working-base-model}
...
model params: 33217 validation loss: 3.942167806625366

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_23_2.png?raw=true)
...
where $q(x_{i})$ is the probability of picking the right word, as estimated by the model. If $q(x_{i})$ is close to 1, then $\logq$ is close to 0; similarly, if $q$ is small, then $\logq$ is a large negative number, so $-\logq$ will be a large positive number. Now to build the intuition: to start, $-\logq=4.17$, so $q=0.015$, or around $\frac{1}{64.715}$. Recall that the vocabulary size $|V|=65$, so what we're basically saying here is that the model is as good at choosing the next letter as randomly picking from our vocabulary. After training, $-\logq=3.93$, so we're now basically choosing between 50 letters. This is a very small improvement, so something is probably wrong.
...
model params: 33217 validation loss: 2.5058052778244018

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_25_2.png?raw=true)
...
## Llama specifics {#llama-specifics}
...
model params: 35265 validation loss: 2.5015316724777223

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_35_2.png?raw=true)
...
![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_39_0.png?raw=true)

Let's make sure these work. They should exhibit the quality that

$$
q_{m}^{T}k_{n}=(R_{\Theta,m}^{d}W_{q}x_{m})^{T}(R_{\Theta,n}^{d}W_{k}x_{n})=x^{T}W_{q}R_{\Theta,n-m}^{d}W_{k}x_{n}.
$$
...
model params: 559681 validation loss: 0.1623048834502697

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_50_2.png?raw=true)

Wow, would you look at that, our validation loss is down to .16. That is so low...it's almost too low. Consider the check we used from above: $\exp(\mathrm{.16})=1.17$, so it's as if the model is choosing basically next character correctly every time. Let's see what happens when we generate.
...
![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_54_1.png?raw=true)
...
![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_57_1.png?raw=true)
...
model params: 559681 validation loss: 2.0815173864364622

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_60_2.png?raw=true)
...
![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_62_2.png?raw=true)
...
$$
\text{Swish}_{\beta}(x)=x\sigma(\betax)
$$
...
model params: 592706 validation loss: 1.8963455319404603

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_65_2.png?raw=true)
...
model params: 2370246 validation loss: 1.5532499313354493

![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_69_2.png?raw=true)
...
![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_71_2.png?raw=true)
...
![png](https://github.com/bkitano/llama-from-scratch/blob/main/llama_files/llama_73_2.png?raw=true)
...
Proforne, wift'es swleen, was no bunderes'd a a quain beath! Tybell is my gateer stalk smen'd as be matious dazest brink thou lord Enves were cIUll, afe and whwas seath This a is, an tale hoice his his onety Meall-tearn not murkawn, fase bettizen'd her, To belacquesterer? baxewed wupl usweggs yet tall An
...
tensor(1.2358, grad\_fn\=)

# Miscellaneous {#miscellaneous}

## Check for Gradient Flows {#check-for-gradient-flows}
...
## Experiment with hyperparams, aka "change the oven settings" {#experiment-with-hyperparams-aka-change-the-oven-settings}
