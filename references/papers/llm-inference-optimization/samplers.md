...
description: An idiot's comprehensive guide to modern sampling

# Dummy's Guide to Modern LLM Sampling {#dummys-guide-to-modern-llm-sampling}

1. [Intro Knowledge](#intro-knowledge)
    1. [Short Glossary](#short-glossary)
    2. [Why tokens?](#why-tokens)
        1. [Why not letters?](#why-not-letters)
        2. [Why not whole words?](#why-not-whole-words)
        3. [How are the sub-words chosen?](#how-are-the-sub-words-chosen)
    3. [How does the model generate text?](#how-does-the-model-generate-text)
    4. [From Tokens to Text: How LLMs Generate Content](#from-tokens-to-text-how-llms-generate-content)
2. [Sampling](#sampling)
    1. [Notes on Algorithm Presentations](#notes-on-algorithm-presentations)
        1. [Notation Guide](#notation-guide)
        2. [Implementation Considerations](#implementation-considerations)
    2. [Temperature](#temperature)
    3. [Presence Penalty](#presence-penalty)
    4. [Frequency Penalty](#frequency-penalty)
    5. [Repetition Penalty](#repetition-penalty)
    6. [DRY (Don't Repeat Yourself)](#dry-dont-repeat-yourself)
    7. [Top-K](#top-k)
    8. [Top-P](#top-p)
    9. [Min-P](#min-p)
    10. [Top-A](#top-a)
    11. [XTC (eXclude Top Choices)](#xtc-exclude-top-choices)
    12. [Top-N-Sigma](#top-n-sigma)
    13. [Tail-Free Sampling](#tail-free-sampling)
    14. [Eta Cutoff](#eta-cutoff)
    15. [Epsilon Cutoff](#epsilon-cutoff)
    16. [Locally Typical Sampling](#locally-typical-sampling)
    17. [Quadratic Sampling](#quadratic-sampling)
    18. [Mirostat Sampling](#mirostat-sampling)
    19. [Dynamic Temperature Sampling](#dynamic-temperature-sampling)
    20. [Beam Search](#beam-search)
    21. [Contrastive Search](#contrastive-search)
3. [Sampler Order](#sampler-order)
    1. [The Typical Sampling Pipeline](#the-typical-sampling-pipeline)
    2. [Effects and Interactions of Samplers with Each Other](#effects-and-interactions-of-samplers-with-each-other)
        1. [How Samplers Transform Distributions](#how-samplers-transform-distributions)
        2. [Critical Order-Dependent Interactions](#critical-order-dependent-interactions)
            1. [Temperature Before vs. After Filtering](#temperature-before-vs-after-filtering)
            2. [Penalties Before vs. After Other Samplers](#penalties-before-vs-after-other-samplers)
            3. [DRY's Position Matters](#drys-position-matters)
        3. [Synergies and Conflicts](#synergies-and-conflicts)
            1. [Synergistic Combos](#synergistic-combos)
            2. [Conflicting Combos](#conflicting-combos)
4. [Advanced: More on Tokenizers](#advanced-more-on-tokenizers)
    1. [Building the Vocabulary](#building-the-vocabulary)
        1. [Byte Pair Encoding (BPE)](#byte-pair-encoding-bpe)
        2. [SentencePiece](#sentencepiece)
        3. [Vocabulary Size and Granularity](#vocabulary-size-and-granularity)
        4. [Token Boundaries and Phrasing](#token-boundaries-and-phrasing)
        5. [Handling Rare or New Words](#handling-rare-or-new-words)

## Intro Knowledge {#intro-knowledge}
...
#### How are the sub-words chosen? {#how-are-the-sub-words-chosen}
...
### From Tokens to Text: How LLMs Generate Content {#from-tokens-to-text-how-llms-generate-content}
...
As explained above, LLMs will pick the most probable token to generate. Sampling is the practice of introducing *controlled randomness*. With pure "greedy" sampling, it would pick the #1 option every time, but that's boring! We use sampling methods like temperature, penalties, or truncation to allow for a bit of creative variation. This document will go through every popular sampling method, and explains how all of them work; both from a simple-to-understand and technical perspectives.

### Notes on Algorithm Presentations {#notes-on-algorithm-presentations}
...
### XTC (eXclude Top Choices) {#xtc-exclude-top-choices}
...
### Quadratic Sampling {#quadratic-sampling}
...
### Dynamic Temperature Sampling {#dynamic-temperature-sampling}
...
### The Typical Sampling Pipeline {#the-typical-sampling-pipeline}

A typical sampling pipeline in production LLM systems follow these sequential steps:

1. **Generate Raw Logits**: The model produces unnormalized logits for each token in the vocabulary.
2. **Apply Token Filtering/Banning**: Remove tokens that shouldn't be considred at all.
3. **Apply Penalties**: Apply repetition, frequency, and presence penalties to discourage repetitive outputs.
4. **Apply Pattern-Based Techniques**: Methods like DRY (Don't Repeat Yourself) are applied to identify and penalize repetitive patterns.
5. **Apply Temperature Scaling**: Temperature is either applied as the first or the last sampler (outside penalties and post-softmax samplers), depending on the implementation. For most tasks, temperature is applied first. For creative writing, it is usually last.
6. **Apply Distribution-Shaping Methods**: Techniques like Top-K, Top-P, Min-P, etc. that filter or reshape the probability distribution.
7. **Sample from the Final Distribution**: After all modifications, select a token from the resulting probability distribution.

### Effects and Interactions of Samplers with Each Other {#effects-and-interactions-of-samplers-with-each-other}
...
#### How Samplers Transform Distributions {#how-samplers-transform-distributions}

To understand sampler interaction, let's first visualize it:

1. **Original Distribution**: A typical token distribution has a few high-probability tokens followed by a long tail of increasingly unlikely options.
2. **After Penalties**: Penalties flatten peaks for previously used tokens, so they elevate alternatives that haven't appeared yet.
3. **After Temperature**: Lower temperatures (\<1.0) sharpen the distribution, making peaks higher and valleys lower. Higher temperatures (>1.0) flatten the distribution.
4. **After Filtering (Top-K/P/etc.)**: These truncate the distribution, removing lowest-probability tokens and often renormalizing the remaining probs.
...
#### Critical Order-Dependent Interactions {#critical-order-dependent-interactions}

##### Temperature Before vs. After Filtering {#temperature-before-vs-after-filtering}
...
##### Penalties Before vs. After Other Samplers {#penalties-before-vs-after-other-samplers}
...
##### DRY's Position Matters {#drys-position-matters}
...
##### Synergistic Combos {#synergistic-combos}

1. **Top-K \+ Top-P**: Top-K provides a hard limit on tokens considered, while Top-P adapts to the confidence of the model. Together they give us guardrails and enough flexibility.
2. **Temperature \+ Min-P**: High temperature flattens the distribution, while Min-P establishes a quality floor relative to the best token. This combination increases creativity while filtering out truly poor options.

##### Conflicting Combos {#conflicting-combos}

1. **High Temperature \+ Low Top-K**: High temperature tries to flatten the distribution, while low Top-K severely restricts options. The Top-K will essentially override much of the temperature's effect.
2. **Multiple Filtering Methods**: Applying Top-K, Top-P, Min-P, and TFS together often results in the most restrictive one dominating, making the others redundant.
3. **XTC \+ Top-A**: Both methods aim to exclude top choices but in different ways. Using both can overly restrict the sampling space.

## Advanced: More on Tokenizers {#advanced-more-on-tokenizers}

The intro gave you some knowledge on the why and how of tokenizers, but it was still rather vague. How the text is broken down into tokens fundamentally shapes the landscape that your samplers navigate. The choice of tokenizer isn't just a technical detail; it subtly influences *what* the model predicts probabilities for, and therefore, what our samplers have to work with.

Think of it like this: you're giving directions. If your map uses only major highways as segments (coarse tokenization), your choices at each junction are going to be quite limited. If your map uses every single street and alley (fine-grained tokenization), you have far more options, but navigating might become overly complex or lead to weird routes, if you're not sufficiently trained in reading maps (just imagine that was a good analogy). The tokenizer defines the "segments" of the language map the LLM uses.

### Building the Vocabulary {#building-the-vocabulary}

Why sub-words? As we mentioned in the intro, pure character-level tokenization leads to very long sequences, while pure word-level tokenization struggles with huge vocabularies and unknown words. Sub-word algorithms strike a balance between the two. Two most popular approaches are BPE and SentencePiece:

#### Byte Pair Encoding (BPE) {#byte-pair-encoding-bpe}

Imagine you've just learned the alphabet of the English language. You look at a huge amount of text and find the most common pair of adjacent letters (say, `t` and `h`). You merge them into a single new unit, `th`. Now you repeat: find the next most common pair (which might involve your new `th` unit, like `th` \+ `e` becoming `the`) and merge again. You keep doing this until you've built up a vocabulary of the desired size (e.g. 32000 or 128256 units), containing common characters, sub-words, and even whole words. It's like automatically creating abbreviations for frequent letter combinations.

If that was too simple for a technical audience to understand: BPE starts with a base vocab of individual characters (or bytes). It iteratively counts all adjacent pairs of symbols in a large training corpus and merges the most frequent pair into a new vocab entry. This new merged symbol replaces the original pair in the corpus, and the process repeats for a predetermined number of merges (controlling the final vocab size). Standard BPE often requires pre-tokenization (e.g. splitting text by spaces and punctuation) and might not handle whitespace consistently across implementations.

#### SentencePiece {#sentencepiece}

SP takes a slightly different approach. Instead of pre-splitting the text, it treats the text as a raw sequence of [Unicode](https://en.wikipedia.org/wiki/Unicode) characters, including spaces. It learns the sub-word units directly from this raw stream. A key is often encoding the space itself as part of the token (e.g. representing `the` as a single token `_the`). This makes the process smoother and language-independent - it doesn't need to know where words start or end beforehand. Think of it as learning punctuation and spacing rules as part of the vocabulary itself.

Since SP operates directly on raw Unicode, it often encodes whitespace explicitly as we mentioned (`U+2581`) as part of the token itself, making tokenization and de-tokenization perfectly reversible and lossless. It can implement different algorithms internally, including BPE or a unigram language model approach (which starts with many possible sub-words and iteratively removes the ones that least harm the overall likelihood of the text according to the language model). It's popular in modern LLMs because of its language-agnostic nature and reversibility. For example, Google used WordPiece at one point (not sure if they still do?), which is similar to BPE but merges based on maximizing likelihood rather than frequency, and SP can be used as a wrapper for it.

#### Vocabulary Size and Granularity {#vocabulary-size-and-granularity}

Tokenizers have a fixed vocab size. Some use larger vocabs with more whole words, while others use smaller vocabs relying heavily on sub-word units.

A tokenizer that keeps common words like "sampling" or "probability" as single token means the model predicts the likelihood of the *entire concept* at once. This can sometimes lead to more direct, predictable outputs for common phrases. But it struggles more with rare words (forcing `<UNK>` tokens or awkward sub-word combinations learned via BPE/SP) and might offer fewer "slightly different" choices for sampler to pick from.

Breaking "sampling" into `sampl` \+ `ing` means the model predicts probabilities for `sampl` first, then `ing`. This offers more granular control and flexibility - it gives the model room to switch its prediction from `sampling` to `sampler` if the sampler function we're using (e.g. temperature) allows it., since samplers have more nuanced choices at each step. This approach handles rare words better by constructing them from pieces. The downside? It can sometimes lead to slightly less coherent or "stuck" outputs if the sampler gets fixated on a common sub-word prefix, and penalties like DRY need to track shorter, potentially less meaningful sequences.

#### Token Boundaries and Phrasing {#token-boundaries-and-phrasing}

Different tokenizers might split the same phrase differently based on their learned merges or rules. "State-of-the-art" could become `State` \+ `-` \+ `of` \+ `-` \+ `the` \+ `-` \+ `art`, or perhaps `State` \+ `_of` \+ `_the` \+ `_art` (SP style), or even `State-of-the-art` if it was frequent enough to become a single token during BPE merges.

The way phrases are split directly impacts n-gram based samplers like DRY. If "once upon a time" is tokenized as `once` `upon` `a` `time`, DRY can easily spot and penalize that 4-gram. If it's `_once` `_upon` `_a` `_time` (SP style), the patterns looks different but is still trackable. Now if it was a single word, then it's essentially impossible to penalize it without rolling back the output. Different splits change the exact n-grams being monitored.

It also affects the probability distribution itself. If `_of-the-art` is a single token, the model predicts its likelihood directly. If it's multiple tokens, the model predicts `_of`, then `_the`, etc., and sampling happens at each of those finer steps, potentially allowing samplers to steer the generation mid-phrase.

#### Handling Rare or New Words {#handling-rare-or-new-words}

When encountering a word not explicitly in its vocab (or seen rarely), word-level tokenizers might use `<UNK>`. BPE and SP break it down into known pieces (e.g., "tokenization" -> `token` \+ `ization` or `_token` \+ `ization`). If a tokenizer produces `<UNK>`, samplers will have one less meaningful option to consider. If it uses sub-words, samplers can creatively combine these pieces. But generating a rare word might take multiple sampling steps, increasing the chance of going off-track compared to generating a single, known token. Penalty mechanisms also need to track these sub-word sequences. SP's reversibility ensures even unknown words can be represented and construction.
