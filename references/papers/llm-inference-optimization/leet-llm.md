title: GitHub - videlalvaro/inference-school: A hands-on Swift and Metal course for building LLM inference from first principles on Apple silicon, with 48 guided lessons, runnable exercises, a native macOS Studio, and a complete companion book.
description: A hands-on Swift and Metal course for building LLM inference from first principles on Apple silicon, with 48 guided lessons, runnable exercises, a native macOS Studio, and a complete companion book. - videlalvaro/inference-school

# GitHub - videlalvaro/inference-school: A hands-on Swift and Metal course for building LLM inference from first principles on Apple silicon, with 48 guided lessons, runnable exercises, a native macOS Studio, and a complete companion book.
...
[![Inference School Studio displaying the Start Here lesson and token-generation diagram](https://github.com/videlalvaro/inference-school/raw/main/docs/assets/inference-school-studio-start-here.png)](https://github.com/videlalvaro/inference-school/blob/main/docs/assets/inference-school-studio-start-here.png)
...
It opens at the reader-only orientation lesson `000`. Read [Start Here](https://github.com/videlalvaro/inference-school/blob/main/docs/START-HERE.md) for setup, local-execution boundaries, and the intended learning loop. Problems 001 through 047 are runnable, including the final profiling and systems module.
...
The complete course is available as a 545-page PDF and a reflowable EPUB with native diagrams, exercises, and chapter-end solutions:

[Download the Inference School Companion PDF](https://github.com/videlalvaro/inference-school/blob/main/dist/Inference-School-Companion.pdf)

[Download the Inference School Companion EPUB](https://github.com/videlalvaro/inference-school/blob/main/dist/Inference-School-Companion.epub)
...
The book toolchain requires Python 3, Pandoc, LuaLaTeX, latexmk, and Ghostscript. The complete verification command also requires EPUBCheck. Intermediate book files remain excluded from Git; the published PDF and EPUB are versioned so readers can download them directly from the repository.

This repository contains the complete runnable 47-problem curriculum and its educational capstone inference engine:
...
- Vector-dot, roofline, KV-layout, fused-Q4-GEMV, buffer-plan, fused-QKV, prefill/decode profile, and capstone report commands

The capstone runs the complete educational model on the CPU reference backend and executes fused QKV plus RoPE as an explicitly labeled Metal verification slice. It does not claim a pretrained model or complete Metal generation.
...
Learner checks are expected to fail value cases before you implement an exercise. For problems with a Metal stage, implement both the Swift exercise and its matching starter kernel, then run:
...
Start with [Problem 000](https://github.com/videlalvaro/inference-school/blob/main/Problems/000-start-here/README.md), which defines the course workflow and core model vocabulary. Then use [Anatomy of One Token](https://github.com/videlalvaro/inference-school/blob/main/docs/ONE-TOKEN.md) as a compact map of the complete inference path before the course takes it apart. Keep the [Math Primer](https://github.com/videlalvaro/inference-school/blob/main/docs/MATH-PRIMER.md) nearby and read only the section needed for the current problem.

The complete sequence is in [The Curriculum](https://github.com/videlalvaro/inference-school/blob/main/docs/CURRICULUM.md). It moves through:
...
The teaching and completion rules are documented in [docs/COURSE-DESIGN.md](https://github.com/videlalvaro/inference-school/blob/main/docs/COURSE-DESIGN.md).

Inference School executes learner code. The packaged Studio confines that work to an App Sandbox and a user-selected folder, but command-line checks inherit the permissions of the terminal that launches them. Read [Start Here](https://github.com/videlalvaro/inference-school/blob/main/docs/START-HERE.md) before running checks, and report vulnerabilities according to [SECURITY.md](https://github.com/videlalvaro/inference-school/blob/main/SECURITY.md).

Inference School is licensed under the [Apache License 2.0](https://github.com/videlalvaro/inference-school/blob/main/LICENSE). Dependency attributions are recorded in [THIRD\_PARTY\_NOTICES.md](https://github.com/videlalvaro/inference-school/blob/main/THIRD_PARTY_NOTICES.md) and included in packaged Studio builds.
