...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0002-00.png)


**----- Start of picture text -----**<br>
Performance on Benchmarks Speed on Stages<br>13.0<br>Full Attention 11.6×<br>0.5 NSA 11.0<br>9.0×<br>9.0<br>0.4<br>7.0<br>0.3 6.0×<br>5.0<br>0.2<br>3.0<br>0.1<br>1.0<br>0.0<br>General LongBench Reasoning Decode Forward Backward<br>Score<br>Speedup Ratio<br>**----- End of picture text -----**<br>
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0003-00.png)


**----- Start of picture text -----**<br>
Native Sparse Attention Mechanism<br>Split to Continuous Blocks<br>Compressed Attention Mask<br>Compress Selection<br>Compression<br>Sliding Selected Attention Mask<br>Compressed Attention Selected Attention Sliding Attention<br>Sliding Attention Mask<br>Output Output Output<br>Gated Output Attention Score Query Token Activated Token Evicted Token Ignored Token<br>...<br>Concat<br>Top-n Selection<br>**----- End of picture text -----**<br>
...
**Attention Mechanism** is widely used in language modeling where each query token **q** _𝑡_ computes relevance scores against all preceding keys **k** : _𝑡_ to generate a weighted sum of values **v** : _𝑡_ . Formally, for an input sequence of length _𝑡_ , the attention operation is defined as: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0005-07.png)


where Attn denotes the attention function: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0005-09.png)


Here, _𝛼𝑡_ , _𝑖_ represents the attention weight between **q** _𝑡_ and **k** _𝑖_ , and _𝑑𝑘_ is the feature dimension of keys. As sequence length increases, attention computation becomes increasingly dominant in the overall computational cost, presenting significant challenges for long-context processing. 
...
To leverage the potential of attention with natural sparse pattern, we propose replacing the original key-value pairs **k** : _𝑡_ , **v** : _𝑡_ in Equation (1) with a more compact and information-dense set of representation key-value pairs _𝐾_[˜] _𝑡_ , _𝑉_[˜] _𝑡_ given each query **q** _𝑡_ . Specifically, we formally define the optimized attention output as follows: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0006-03.png)



![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0006-04.png)


where _𝐾_[˜] _𝑡_ , _𝑉_[˜] _𝑡_ are dynamically constructed based on the current query **q** _𝑡_ and the contextual memory **k** : _𝑡_ , **v** : _𝑡_ . We can design various mapping strategies to get different categories of _𝐾_[˜] _𝑡[𝑐]_[,] _[𝑉]_[˜] _𝑡[𝑐]_[,] and combine them as follows: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0006-06.png)


As illustrated in Figure 2, NSA have three mapping strategies C = {cmp, slc, win}, representing compression, selection, and sliding window for keys and values. _𝑔𝑡[𝑐]_[∈[][0, 1][]][is the gate score for] corresponding strategy _𝑐_ , derived from input features via an MLP and sigmoid activation. Let _𝑁𝑡_ denote the total number of remapped keys/values: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0006-08.png)


We maintain a high sparsity ratio by ensuring _𝑁𝑡_ ≪ _𝑡_ . 

## **3.3. Algorithm Design** 

In this subsection, we introduce the design of our remapping strategies _𝑓𝐾_ and _𝑓𝑉_ : token compression, token selection, and sliding window. 

## _**3.3.1. Token Compression**_ 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0006-14.png)


where _𝑙_ is the block length, _𝑑_ is the sliding stride between adjacent blocks, and _𝜑_ is a learnable MLP with intra-block position encoding to map keys in a block to a single compressed key. ˜ _𝐾𝑡_[cmp] ∈ **R** _[𝑑][𝑘]_[×][⌊] _[𝑡]_[−] _𝑑[𝑙]_[⌋] is tensor composed by compresion keys. Usually, we adopt _𝑑< 𝑙_ to mitigate information fragmentation. An analogous formulation holds for the compressed value representation _𝑉_[˜] _𝑡_[cmp] . Compressed representations capture coarser-grained higher-level semantic information and reduce computational burden of attention. 

6 

## _**3.3.2. Token Selection**_ 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0007-05.png)


˜ where **p**[cmp] _𝑡_ ∈ **R**[⌊] _[𝑡]_[−] _𝑑[𝑙]_[⌋][+][1] is the attention scores between _𝑞𝑡_ and compression keys _𝐾𝑡_[cmp] . Let _𝑙_[′] denote the selection block size. When compression blocks and selection blocks share the same blocking scheme, i.e., _𝑙_[′] = _𝑙_ = _𝑑_ , we can directly obtain the selection block importance scores **p**[slc] _𝑡_ by **p**[slc] _𝑡_ = **p**[cmp] _𝑡_ straightforwardly. For cases where the blocking schemes differ, we derive the importance scores for selection blocks according to their spatial relationship. Given _𝑙_ ⩽ _𝑙_[′] , _𝑑_ | _𝑙_ and _𝑑_ | _𝑙_[′] , we have: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0007-07.png)
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0007-09.png)


where ( _ℎ_ ) in the superscript denotes the head index, and _𝐻_ is the number of query heads in each group. This aggregation ensures consistent block selection across heads within the same group. 

7 

**Top-** _**𝑛**_ **Block Selection.** After obtaining the selection block importance scores, We retain _𝑛_ tokens within the top- sparse blocks ranked by block importance scores, formulated as: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0008-01.png)



![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0008-02.png)


where rank(·) denotes the ranking position in descending order, with rank = 1 corresponding to the highest score, I _𝑡_ is the set of selected blocks’ indices, Cat denotes the concatenation operation. _𝐾_[˜] _𝑡_[slc] ∈ **R** _[𝑑][𝑘]_[×] _[𝑛𝑙]_[′] is tensor composed by compresion keys. An analogous formulation applies to the fine-grained value _𝑉_[˜] _𝑡_[slc][.][The][selected][keys][and][values][then][participate][in][the] attention computation with **q** _𝑡_ as defined in Equation (5). 

## _**3.3.3. Sliding Window**_ 

In attention mechanisms, local patterns typically adapt faster and can dominate the learning process, potentially preventing the model from effectively learning from compression and selection tokens. To address this issue, we introduce a dedicated sliding window branch that explicitly handles local context, allowing other branches (compression and selection) to focus on learning their respective features without being shortcutted by local patterns. Specifically, we maintain recent tokens _𝐾_[˜] _𝑡_[win] = **k** _𝑡_ − _𝑤_ : _𝑡_ , _𝑉_[˜] _𝑡_[win] = **v** _𝑡_ − _𝑤_ : _𝑡_ in a window _𝑤_ , and isolate attention computations of different information sources (compression tokens, and selected tokens, sliding window) into separate branches. These branch outputs are then aggregated through a learned gating mechanism. To further prevent shortcut learning across attention branches with marginal computational overhead, we provide independent keys and values for three branches. This architectural design enables stable learning by preventing gradient interference between local and long-range pattern recognition, while introducing minimal overhead. 

After obtaining all three categories of keys and values ( _𝐾_[˜] _𝑡_[cmp] , _𝑉_[˜] _𝑡_[cmp] ; _𝐾_[˜] _𝑡_[slc][,] _[𝑉]_[˜] _𝑡_[slc][; and] _[𝐾]_[˜] _𝑡_[win] , _𝑉_[˜] _𝑡_[win] ), we compute the final attention output following Equation (5). Together with the compression, selection, and sliding window mechanisms described above, this forms the complete algorithmic framework of NSA. 
...
1. **Group-Centric Data Loading** . For each inner loop, load all heads’ queries _𝑄_ ∈ **R**[[] _[ℎ]_[,] _[𝑑][𝑘]_[]] in the group at position _𝑡_ and their shared sparse key/value block indices I _𝑡_ . 

8 

2. **Shared KV Fetching** . In the inner loop, Sequentially load continuous key/value blocks indexed by I _𝑡_ into SRAM as _𝐾_ ∈ **R**[[] _[𝐵][𝑘]_[,] _[𝑑][𝑘]_[]] , _𝑉_ ∈ **R**[[] _[𝐵][𝑘]_[,] _[𝑑][𝑣]_[]] to minimize memory loading, where _𝐵𝑘_ is the kernel block size satisfying _𝐵𝑘_ | _𝑙_[′] . 

3. **Outer Loop on Grid** . Since the inner-loop length (proportional to the selected block count _𝑛_ ) remains nearly identical for different query blocks, we put query/output loops in Triton’s grid scheduler to simplify and optimize the kernel. 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0009-03.png)


**----- Start of picture text -----**<br>
K Inner Loop<br>V<br>Q<br>Select In<br>Load<br>Select In<br>Load Compute on SRAM<br>Output to HBM<br>Output<br>Grid Loop<br>Grid Loop Inner Loop<br>**----- End of picture text -----**<br>
...
The proposed architecture achieves an effective trade-off between computation cost and model performance. For NSA, we set compression block size _𝑙_ = 32, sliding stride _𝑑_ = 16, selected block size _𝑙_[′] = 64, selected block count _𝑛_ = 16 (including fixed activating the 1 initial block and 2 local blocks), and sliding window size _𝑤_ = 512. Both Full Attention and sparse attention models are pretrained on 270B tokens of 8k-length texts, followed by continued training and supervised fine-tuning on 32k-length texts with YaRN (Peng et al., 2024) to achieve long-context adaptation. Both models are trained to full convergence to ensure fair comparison. As shown in Figure 4, the pretraining loss curve of our NSA and Full Attention baseline demonstrates stable and smooth decline, with NSA consistently outperforming the Full Attention model. 

## **4.2. Baselines Methods** 

In addition to comparing with Full Attention, we evaluate several state-of-the-art inferencestage sparse attention methods: H2O (Zhang et al., 2023b), infLLM (Xiao et al., 2024a), Quest _𝑛_ (Tang et al., 2024), and Exact-Top, which first computes full attention score and select the topscores keys corresponding to each query and then calculates attention on these positions. These 

10 

|Model|SQA<br>MFQA-en MFQA-zh Qasper|SQA<br>MFQA-en MFQA-zh Qasper|SQA<br>MFQA-en MFQA-zh Qasper||MQA|MQA|Dur|Synthetic<br>PassR-en PassR-zh|Synthetic<br>PassR-en PassR-zh|Code<br>LCC|Avg.|
|---|---|---|---|---|---|---|---|---|---|---|---|
|||||HPQ|2Wiki GovRpt|||||||
|H2O<br>InfLLM<br>Quest<br>Exact-Top<br>Full Attn|0.428<br>0.474<br>0.495<br>0.502<br>**0.512**|0.429<br>0.517<br>0.561<br>0.605<br>0.623|0.308<br>0.356<br>0.365<br>0.397<br>0.409|0.112<br>0.306<br>0.295<br>0.321<br>0.350|0.101<br>0.250<br>0.245<br>0.288<br>0.305|0.231<br>0.277<br>0.293<br>0.316<br>**0.324**|0.208<br>0.257<br>0.257<br>0.291<br>0.294||0.704<br>0.421<br>0.766<br>0.486<br>0.792<br>0.478<br>0.810<br>0.548<br>0.830<br>**0.560**|0.092 <br>0.143 <br>0.135 <br>0.156 <br>0.163|0.303<br> 0.383<br> 0.392<br> 0.423<br>0.437|
|NSA|0.503|**0.624**|**0.432**|**0.437**|**0.356**|0.307|**0.341**||**0.905**<br>0.550|**0.232 **|**0.469**|
...
methods span diverse sparse attention paradigms, including KV-cache eviction, query-aware _𝑛_ selection, and exact top- sparse selection. 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0012-00.png)


**----- Start of picture text -----**<br>
Needle in A Haystack 27B NSA 64k Context<br>1.0<br>0<br>11<br>0.8<br>22<br>33<br>0.6<br>44<br>55<br>0.4<br>66<br>77<br>0.2<br>88<br>100<br>0.0<br>1K 8K 15K 22K 29K 36K 43K 50K 57K 64K<br>Depth Percent (%)<br>**----- End of picture text -----**<br>
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0013-02.png)


**----- Start of picture text -----**<br>
Forward Time Comparison Backward Time Comparison<br>1000 FlashAttention-2 2500<br>Sparse Attention Kernel<br>Compression and<br>800 Window Attention 2000<br>9.0 × Speedup Ratio<br>600 1500 6.0×<br>9.0×<br>400 1000<br>200 500<br>6.3× 3.4×<br>2.1× 3.8× 1.1× 2.0×<br>0 0<br>8k 16k 32k 64k 8k 16k 32k 64k<br>Time (ms)<br>**----- End of picture text -----**<br>
...
We evaluate the computational efficiency of NSA against Full Attention on an 8-GPU A100 system. In efficiency analysis, we also configure the model with GQA group _𝑔_ = 4, heads per group _ℎ_ = 16, query/key dimension _𝑑𝑘_ = 192, and value dimension _𝑑𝑣_ = 128. Following the same settings in Section 4, we set NSA compression block size _𝑙_ = 32, sliding stride _𝑑_ = 16, selected block size _𝑙_[′] = 64, selected block count _𝑛_ = 16, and sliding window size _𝑤_ = 512. 
...
The decoding speed of Attention is primarily determined by the memory access bottleneck, which is closely tied to the amount of KV cache loading. In each decoding step, Our NSA just needs to load at most � _𝑠_ − _𝑑 𝑙_ � compression tokens, _𝑛𝑙_ ′ selected tokens, and _𝑤_ neighbor tokens, where _𝑠_ is the cached sequence length. As shown in Table 4, our method exhibits a significant reduction in latency as the decoding length increases, achieving up to 11.6× speedup at 64k context-length. This advantage in memory access efficiency also amplifies with longer sequences. 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0015-00.png)


**----- Start of picture text -----**<br>
Training Loss Curve Visualization for Attention Score Block Pattern<br>3.2<br>Full Attention<br>Auxiliary Loss-based Learnable Selection<br>3.0 Heuristic Parameter-free Selection<br>NSA<br>2.8<br>2.6<br>2.4<br>position<br>2.2<br>0 5000 10000 15000 20000 25000 30000<br>step Figure 8 | Visualization of Attention Map | Visualization of Attention Map Visualization of Attention Map<br>position<br>loss<br>**----- End of picture text -----**<br>


Figure 8 | Visualization of Attention Map | Visualization of Attention Map Visualization of Attention Map on a Full Attention transformer. Lightcolored regions indicate higher attention values. As shown in the figure, attention scores exhibit blockwise clustering distribution. 
...
- C. Xiao, P. Zhang, X. Han, G. Xiao, Y. Lin, Z. Zhang, Z. Liu, and M. Sun. Inf m: Training-free long-context extrapolation for llms with an efficient context memory. In The Thirty-eighth Annual Conference on Neural Information Processing Systems, 2024a. 
...
“|User|Let _𝑥_ , _𝑦_ and _𝑧_ be positive real numbers that satisfy the following system of equations: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0021-03.png)


4 3 2 _𝑚_ Then the value of ��log2( _𝑥 𝑦 𝑧_ )�� is _𝑛_[where] _[ 𝑚]_[and] _[ 𝑛]_[are relatively prime positive integers.] Find _𝑚_ + _𝑛_ . Please reason step by step, and put your final answer within ~~.~~ |Assistant|” 

## **Our Result:** 

⟨think⟩...⟨/think⟩ (2275 tokens) Given the system of equations: 

1. log2 � _𝑦𝑧𝑥_ � = 2[1][2.][log] 2 � _𝑥𝑧𝑦_ � = 13[3.][log] 2 � _𝑥𝑦𝑧_ � =[1] 4 We convert these logarithmic equations to their exponential forms: 

1. _𝑦𝑧𝑥_[=] ~~√~~ 2 2. _𝑥𝑧𝑦_[=][ 2][1][/][3][ 3.] _𝑥𝑦𝑧_[=][ 2][1][/][4] Expressing each variable in terms of _𝑥_ , _𝑦_ , and _𝑧_ : 

1. _𝑥_ = ~~√~~ 2 · _𝑦𝑧_ 2. _𝑦_ = 2[1][/][3] · _𝑥𝑧_ 3. _𝑧_ = 2[1][/][4] · _𝑥𝑦_ Substituting these expressions into each other, we find: 

- From _𝑥_ = ~~√~~ 2 · _𝑦𝑧_ and _𝑦_ = 2[1][/][3] · _𝑥𝑧_ , we get _𝑥_ = ~~√~~ 2 · (2[1][/][3] · _𝑥𝑧_ ) · _𝑧_ , leading to _𝑧_ = 2[−][5][/][12] . - Substituting _𝑧_ into the equation for _𝑦_ , we get _𝑦_ = 2[−][3][/][8] . - Substituting _𝑧_ and _𝑦_ into the equation for _𝑥_ , we get _𝑥_ = 2[−][7][/][24] . 

Next, we compute _𝑥_[4] _𝑦_[3] _𝑧_[2] : - _𝑥_[4] = (2[−][7][/][24] )[4] = 2[−][7][/][6] - _𝑦_[3] = (2[−][3][/][8] )[3] = 2[−][9][/][8] - _𝑧_[2] = (2[−][5][/][12] )[2] = 2[−][5][/][6] Multiplying these together: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0021-12.png)


Taking the logarithm base 2: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0021-14.png)


The absolute value is: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0021-16.png)


21 

Thus, _𝑚_ = 25 and _𝑛_ = 8, giving _𝑚_ + _𝑛_ = 33. 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0022-04.png)


We convert these equations to exponential form: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0022-06.png)


Substituting these into each other, we find: 

1 1 1. _𝑥_ = ~~√~~ 2 · _𝑦𝑧_ 2. _𝑦_ = 2 3 · _𝑥𝑧_ 3. _𝑧_ = 2 4 · _𝑥𝑦_ By solving these equations step-by-step, we find: - _𝑧_ = 2[−][5][/][12] - _𝑥𝑦_ = 2[−][2][/][3] - _𝑦_ = 2[−][1][/][24] - _𝑥_ = 2[1][/][24] 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0022-10.png)


Let _𝐴_ = log2( _𝑥_ ), _𝐵_ = log2( _𝑦_ ), _𝐶_ = log2( _𝑧_ ). The system becomes: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0022-12.png)
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0023-01.png)


Thus, we have: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0023-03.png)


4 3 2 We need to find ��log2( _𝑥 𝑦 𝑧_ )��: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0023-05.png)


Converting to a common denominator (48): 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0023-07.png)


Summing these: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0023-09.png)


The fraction[115] 16[is in simplest form, so] _[ 𝑚]_[=][ 115 and] _[ 𝑛]_[=][ 16.][Therefore,] _[𝑚]_[+] _[ 𝑛]_[=] 115 + 16 = 131. 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0023-11.png)


## **Prompt:** 

“|User |Real numbers _𝑥_ and _𝑦_ with _𝑥_ , _𝑦>_ 1 satisfy log _𝑥_ ( _𝑦[𝑥]_ ) = log _𝑦_ ( _𝑥_[4] _[𝑦]_ ) = 10. What is the value of _𝑥𝑦_ ? Please reason step by step, and put your final answer within ~~.~~ |Assistant|” 
...
1. log _𝑥_ ( _𝑦[𝑥]_ ) = 10 2. log _𝑦_ ( _𝑥_[4] _[𝑦]_ ) = 10 
...
![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0024-03.png)


Let _𝑧_ = log _𝑦_ ( _𝑥_ ). Then, log _𝑥_ ( _𝑦_ ) =[1] _𝑧_[.][Substituting this into the equation, we get:] 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0024-05.png)


For the second equation: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0024-07.png)


Using the power rule, we get: 

_𝑦_ · 4 log _𝑦_ ( _𝑥_ ) = 10 =⇒ 4 _𝑦_ log _𝑦_ ( _𝑥_ ) = 10 Since log _𝑦_ ( _𝑥_ ) = _𝑧_ , this simplifies to: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0024-10.png)


We now have two expressions: 1. _𝑥_ = 10 _𝑧_ 2. _𝑦_ =[5][/] _𝑧_[2] 

Substituting _𝑥_ = 10 _𝑧_ and _𝑦_ =[5][/] _𝑧_[2][into the product] _[𝑥𝑦]_[, we get:] 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0024-13.png)


Thus, the value of _𝑥𝑦_ is 25 . 

## **Baseline Result:** 

⟨think⟩...⟨/think⟩ (16223 tokens) 

Given the problem where real numbers _𝑥_ and _𝑦_ with _𝑥_ , _𝑦>_ 1 satisfy the equations log _𝑥_ ( _𝑦[𝑥]_ ) = 10 and log _𝑦_ ( _𝑥_[4] _𝑦_ ) = 10, we need to find the value of _𝑥𝑦_ . 
...
> _[𝑦]_ Using the change of base formula, log _𝑥_ ( _𝑦_ ) =[ln] ln _𝑥_[.][Since][log] _𝑦[𝑥]_[=] 4[9][,][we][have] log _𝑥 𝑦_ =[4] 9[.][Thus,] 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0025-03.png)


2. log _𝑦_ ( _𝑥_[4] _𝑦_ ) = 10 

4 log _𝑦 𝑥_ + 1 = 10 =⇒ 4 log _𝑦 𝑥_ = 9 =⇒ log _𝑦 𝑥_ =[9] 4 

Again, using the change of base formula, log _𝑥 𝑦_ =[4] 9[.] 

Next, we express _𝑥_ and _𝑦_ in terms of each other: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0025-08.png)


We then compute the product _𝑥𝑦_ : 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0025-10.png)


However, upon re-evaluating and using another approach by setting _𝑥_ = 2 _[𝑎]_ and _𝑦_ = 2 _[𝑏]_ , we find: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0025-12.png)



![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0025-13.png)


Finally, we find that the product _𝑥𝑦_ simplifies to: 


![](references/papers/llm-inference-optimization/2502.11089_images/2502.11089.pdf-0025-15.png)
