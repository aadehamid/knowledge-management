...
- To generate token $t_{i+1} t_{i+1}$, the model must consider the entire sequence from $t_{0} t_0$ to $t_{i} t_i$. From the first instance in the above example $t_{i+1} t_{i+1}$ would be `the` , while all the previous tokens $t_{0} t_0$ to $t_{i} t_i$ would be `[What, is, in]`.
...
For a sequence of $T T$ input embeddings represented as $X\in\mathbb{R}^{T\timesD} X \in \mathbb{R}^{T \times D}$, self-attention is computed as:

- $Q=XW_{Q} Q = XW_Q$, with $W_{Q}\in\mathbb{R}^{D\timesD_{q}} W_Q \in \mathbb{R}^{D \times D_q}$
- $K=XW_{K} K = XW_K$, with $W_{K}\in\mathbb{R}^{D\timesD_{k}} W_K \in \mathbb{R}^{D \times D_k}$
- $V=XW_{V} V = XW_V$, with $W_{V}\in\mathbb{R}^{D\timesD_{v}} W_V \in \mathbb{R}^{D \times D_v}$
- Causal mask $M M$ to prevent future token access

The final output is:

$$\text{Attention}(X;Q,K,V)=\text{softmax}(\frac{QK^{⊤}\cdotM}{\sqrt{d_{k}}})V
...
In autoregressive generation, the model generates one token at a time. With each step, it recomputes $Q Q$, $K K$, and $V V$ for **the entire sequence**, even though the earlier tokens haven’t changed.
...
These checks show that for all but the newest token, $K K$ and $V V$ are identical to previously computed values.
...
- After processing the initial prompt, we **cache** the computed keys ( $K K$ ) and values ( $V V$ ) for each layer.
- During generation, we **only compute** $K K$ **and** $V V$ **for the new token**, and **append** them to the cache.
- We compute $Q Q$ for the current token and use it with the **cached $K K$ and $V V$** to get the output.
...
Previously, the model recomputed $K K$ and $V V$ at every generation step. Now we only compute $K_{\text{new}} K_{\text{new}}$, $V_{\text{new}} V_{\text{new}}$ for the current token, and append them to the cached values.
