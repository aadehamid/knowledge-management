title: Taking PyTorch for Granted | wh

# Taking PyTorch for Granted {#post-title}

### *Understanding PyTorch without the abstractions*
...
## But what is a tensor? {#but-what-is-a-tensor}

In the context of Machine Learning, a tensor is a $n$-dimensional matrix. Okay, but what is a `torch.Tensor`? Specifically, what actually happens when the following piece of code is run: `a = torch.tensor(1.0, requires_grad=True)`. It turns out that PyTorch will allocate the data on the heap and returns the pointer to that data as a shared pointer ^([1](https://discuss.pytorch.org/t/where-does-torch-tensor-create-the-object-stack-or-heap/182753) ). To better understand pointers in PyTorch, let’s look at a few examples.
...
## Implementing Storage {#implementing-storage}
...
Can we do better and have only 1 memory access per item? The key observation is that the $(x,y)$ coordinate scheme is only useful for us to visualise the layout of the matrix, but this is not necessary for the computer. In fact, according to the `torch.Storage` documentation, `torch.Storage` is a “a contiguous, 1-dimensional array of elements”. Then it follows that as long as we can define some function ^([4](https://github.com/nreHieW/r-nn/blob/main/src/core/tensor/mod.rs#L92) )that maps our $(x,y)$ coordinate scheme (or indeed any $n$-dimensional scheme) to this 1 dimensional representation, we would have successfully achieved a implementation with only a single memory access per item. ![Storage Example](https://nrehiew.github.io/blog/pytorch/images/Storage.png)

This representation has several benefits. From a practical standpoint, other than the single memory access required, elements are guaranteed to be adjacent to each other in memory which allows us to make better use of spatial locality ^([5](https://en.wikipedia.org/wiki/Locality_of_reference) )in our cache.
...
![Transpose Example](https://nrehiew.github.io/blog/pytorch/images/Transpose.png)
...
2. Starting from the right most dimensions, the dimensions must have equal sizes, one of them is 1 or one of them does not exist. This might seem confusing at first, so it might be easier to start by understanding how the resultant shape is calculated. The algorithm is as follows:
...
## Broadcasting with Matrix Multiplication {#broadcasting-with-matrix-multiplication}
...
### ++Matrix Multiplication++ {#insmatrix-multiplicationins}
...
### ++Memory and Intermediate activations++ {#insmemory-and-intermediate-activationsins}
